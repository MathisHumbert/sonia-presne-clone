...
void main(){
  ...
  // vec3 color = mix(white, blue, u_color_mix);
  // float x = vUv.x * 1.5; // size
  // float y = vUv.y * 1.5; // size
  // float z = u_time * .005 + .5; // speed
  // float s = .01; // smoothing
  // float l = .1; // line width
  // float noise = perlinNoise(vec3(x, y, z)) + 0.5;
  // noise = frac(noise * 100.); // amount of lines
  
	// float pixelNoiseChange = 0.2;
  ...
  // color = color - heightLine * (1. - u_color_mix * 2.);

  vec3 white = vec3(1.);
  vec3 blue = vec3(.125, .145, .298);

  float offx = vUv.x + sin(vUv.y + uTime * .1);
  float offy = vUv.y - uTime * 0.1 - cos(uTime * .001);

  float noise = snoise(vec3(offx, offy, uTime * 0.1) * 2.);

  noise = smoothstep(noise, 0., 1.);

  vec3 color = mix(blue, white, noise);
...
//     // vec2 uv = gl_FragCoord.xy / u_resolution; // based on screen
//     vec2 uv = vec2(v_texcoord.x + 5., -v_texcoord.y + .5); // based on geometry
    
    
//     // vec3 white = vec3(1.);
//     // vec3 blue = vec3(.125, .145, .298);
//     // vec3 color = mix(white, blue, u_color_mix);
//     // float x = uv.x * 1.5; // size
//     // float y = uv.y * 1.5; // size
//     // float z = u_time * .005 + .5; // speed
//     // float s = .1; // smoothing
...
//     vec3 blue = vec3(.125, .145, .298);
//     vec3 color = mix(white, blue, u_color_mix);
//     float x = uv.x * 11.; // size
//     float y = uv.y * 11.; // size
//     float z = u_time * .01; // speed
//     float s = .1; // smoothing
//     float l = .2; // line width
//     float noise = perlinNoise(vec3(x, y, z)) + 0.5;
//     noise = frac(noise * 11.); // amount of lines
    
//     float pixelNoiseChange = fwidth(noise) * l;
//     float heightLine = smoothstep(1. - s - pixelNoiseChange, 1., noise);
//     heightLine += smoothstep(s + pixelNoiseChange, 0., noise);
//     heightLine = clamp(heightLine, 0., .5);
//     heightLine = heightLine * .065;
